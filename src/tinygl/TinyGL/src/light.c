  ...
  case GL_SPOT_CUTOFF:
    {
      ...
      l->spot_cutoff=a;
      if (a != 180) l->cos_spot_cutoff=zcos(a * M_PI / 180.0);
    }
    break;
...
void gl_shade_vertex(GLContext *c,GLVertex *v)
{
  ...
  for(l=c->first_light;l!=NULL;l=l->next) {
    ...
    if (l->position.v[3] == 0) {
      ...
      dist=zsqrt(d.X*d.X+d.Y*d.Y+d.Z*d.Z);
      if (dist>1E-10f) {
        tmp=1/dist;
        d.X*=tmp;
        d.Y*=tmp;
    ...
    if (dot>0) {
      ...
      if (l->spot_cutoff != 180) {
        ...
        if (dot_spot < l->cos_spot_cutoff) {
          ...
          if (l->spot_exponent > 0) {
            att=att*zpow(dot_spot,l->spot_exponent);
          }
        }
      }
      ...
      if (dot_spec>0) {
        GLSpecBuf *specbuf;
        int idx;
        tmp=zsqrt(s.X*s.X+s.Y*s.Y+s.Z*s.Z);
        if (tmp > 1E-3) {
        ...
        /* dot_spec= zpow(dot_spec,m->shininess);*/
